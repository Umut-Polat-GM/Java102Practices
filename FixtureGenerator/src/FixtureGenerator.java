...
public class FixtureGenerator {
    ...
    public List<List<Match>> getFixtures(List<String> teams) {
        ...
        if (numberOfTeams % 2 != 0) {
            numberOfTeams++;
            teams.add("Bay"); // Eğer tek sayıda takım listesi girilirse 'Bay' isminde
        }
        Collections.shuffle(teams); //rastgele takım belirleyelemek için kullandık.

        int totalRounds = numberOfTeams - 1;      //Her bir takım rakip takımlarla bir maç yapar.
        int matchesPerRound = numberOfTeams / 2;  //Her hafta takım sayısının yarısı kadar mac yapılır.
